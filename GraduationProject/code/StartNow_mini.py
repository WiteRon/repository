...
best_model_path = r"C:\Users\Dennis\Desktop\TFT_model\test_name_2.ckpt"
...
plt.plot(x,act_np,  color='blue', label='act')
plt.plot(x,pre_np,  color='red', label='pre')
plt.plot(x,abs(pre_np-act_np),  color='green', label='gap')

plt.legend()
plt.xlabel('Value')
plt.ylabel('Sequence')
plt.title('Line Plot of Array A and Array B')
plt.show()

#raw_predictions = best_tft.predict(val_dataloader, mode="raw")
# raw_predictions, x = best_tft.predict(val_dataloader, mode="raw", return_x=True)
raw_predictions = best_tft.predict(val_dataloader, return_x=True,mode="raw")
interpretation = best_tft.interpret_output(raw_predictions, reduction="sum")
best_tft.plot_interpretation(interpretation)
# raw_predictions = best_tft.predict(val_dataloader, mode="raw", return_x=True)
# print(raw_predictions._fields)

#print((((actuals - predictions).abs()).numpy()).np.mean(axis=1))
